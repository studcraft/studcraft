# infantry-movement Specification Delta

`openspec/specs/` carries no infantry capability, so every requirement below is
`ADDED` rather than `MODIFIED`. The closest existing capability is `unit-base`,
whose *Unit Base Projections* requirement already carries an unarchived
`MODIFIED` delta from `general-review` (#130); a second delta against the same
requirement is the collision `system/workflow.md` ("When several changes
modified the same requirement") describes. `design.md`, Decision 5.

The requirements are written against `docs/17-infantry.md` as it reads once
`tasks.md` Part B is applied.

**`CORE-001`'s new clause carries no delta of its own.** The living `unit-base`
spec already requires it — *"Every distance, Deployment Volume, and vehicle or
infantry footprint SHALL be expressed using this unit"*, and *"the horizontal
projection (4 × 3 studs) for distances, movement, deployment floors and
footprints"*. Task 1b.1 writes into `docs/` what that requirement already says;
it does not change the requirement.

**`CBT-014` carries no delta either.** No capability covers it, which is the
same fact that retired it: it stated no rule.
