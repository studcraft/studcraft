...
- [ ] 0.1 Work on branch `access-openings-must-pass-the-model` (`openspec/config.yaml` requires one branch per proposal).
...
- [ ] 1.1.1 In `CMP-009`, add one bullet to the existing "Requirements:" list, after "Must physically open and close.":

> - The opening must physically pass the models that use the door (CMP-018).

- [ ] 1.1.2 Change nothing else in `CMP-009`. The "Doors are used for:" list and the Action Point line stay exactly as they are.

### 1.2 `CMP-010` — Ramps

- [ ] 1.2.1 In `CMP-010`, add one bullet to the existing "Requirements:" list, after "Must physically move.":

> - The opening the ramp gives access to must physically pass the models that use it (CMP-018).

- [ ] 1.2.2 Change nothing else in `CMP-010`.

### 1.3 Add `CMP-018` at the end of the rules

- [ ] 1.3.1 Insert the following **after** `CMP-017`'s closing `---` and **before** the `# Summary` heading:
...
- [ ] 2.1.1 In `TRN-007`, insert the following line immediately **after** "Only functional access points may be used." and **before** the "Examples:" line:
...
- [ ] 2.1.2 Leave the examples list and the "Decorative access points have no gameplay effect." line unchanged.

### 2.2 `TRN-011` — Firing Ports

- [ ] 2.2.1 In `TRN-011`, add the following as a new paragraph immediately **after** "The opening determines the firing arc." and **before** the closing `---`:
...
- [ ] 2.2.2 Change nothing else in `TRN-011`. The list of openings — Windows, Gun ports, Roof hatches, Observation slits — stays as it is.
...
- [ ] 3.1.1 Replace the line "Closed doors block movement. Once opened, the doorway becomes a valid movement path." with:
...
- [ ] 3.1.2 Leave the Action Point line unchanged.

### 3.2 `MOVE-019` — Ramps

- [ ] 3.2.1 Replace the line "A lowered ramp immediately becomes usable terrain and is a legal access point (MOVE-011, MOVE-014)." with:
...
- [ ] 3.2.2 Leave the Action Point line unchanged.
...
- [ ] 4.1.1 In `GEO-004`, add one bullet to the existing list, after the Cover bullet:
...
- [ ] 4.1.2 Change nothing else in `GEO-004`. The two paragraphs below the list already explain why Visual Geometry participates in physical checks, and they cover the new bullet without amendment.

### 4.2 Glossary

- [ ] 4.2.1 Add an `## Access Opening` entry at the **end** of the definitions in `docs/14-glossary.md`, immediately after the last existing entry and its `---` separator, and **before** the closing `> **Every Brick Matters.**` line. The glossary is in append order, not alphabetical — verify by reading it, then follow it.

- [ ] 4.2.2 Match the surrounding style: a `## ` heading, one paragraph, a document-and-ID citation, then a `---` separator.
...
- [ ] 5.1 **The spec delta is already written.** `openspec/changes/access-openings-must-pass-the-model/specs/geometry-layers/spec.md` modifies the *Visual Geometry Still Applies to Physical Checks* requirement and carries all three scenarios. Do not edit it. Task 6.9 checks it stays coherent.
- [ ] 5.2 `TRN-015` (Emergency Exit) is untouched. It already states that passengers with no functional access point remaining are trapped, which covers the construction case without amendment.
- [ ] 5.3 `TRN-003`, `TRN-004` and `CMP-012` are untouched. Interior capacity and layout are a separate question from passage.
- [ ] 5.4 `TODO.md` is not edited. Nothing here declares a gap; this change closes one.
- [ ] 5.5 `CMP-001`'s list of functional components is untouched. Doors and ramps stay on it — the change adds a requirement, it does not remove the category.
- [ ] 5.6 No Action Point cost changes anywhere. Opening a door still costs 1 AP (`CORE-007`), embarking still costs 1 AP per Unit Base (`TRN-005`).

---

## 6. Verify

- [ ] 6.1 Run `python3 scripts/lint_ruleset.py`; confirm no structural issues. This also checks that every `(CMP-NNN)`, `(TRN-NNN)`, `(MOVE-NNN)`, `(GEO-NNN)` and `(VEH-NNN)` reference added above resolves.
- [ ] 6.2 Run `grep -rcE '^#{1,2} [A-Z]{3,4}-[0-9]{3} ' docs/ | awk -F: '{s+=$2} END {print s}'` and confirm **220** — one more than the 219 before, that one being `CMP-018`.
- [ ] 6.3 Run `grep -n "^# CMP-" docs/05-construction-components.md` and confirm `CMP-001` through `CMP-018` appear once each, ascending, no gaps.
- [ ] 6.4 Run `grep -rn "CMP-018" docs/` and confirm it is cited from `CMP-009`, `CMP-010`, `TRN-007`, `TRN-011`, `MOVE-018`, `MOVE-019`, `GEO-004` and the glossary — eight citations plus the rule's own heading and its internal self-references.
- [ ] 6.5 Run `grep -rn "minifigure is roughly\|1 × 2 tile\|not against the approach" docs/` and confirm each phrase appears **once**, all inside `CMP-018`. The reasoning must not be duplicated into the citing rules.
- [ ] 6.6 Run `grep -rn "line of sight, not a model" docs/` and confirm exactly one hit, in `TRN-011`. Without it, this change reads as invalidating every observation slit in the game.
- [ ] 6.7 Confirm no numeric value changed anywhere in `docs/`. The `4 studs` and `4 bricks` in `CMP-018` are new illustrative text, not a changed value.
- [ ] 6.8 Run `git diff --stat main...HEAD` and confirm exactly these paths changed: the five documents listed under **Scope**, plus the five files under `openspec/changes/access-openings-must-pass-the-model/`.
- [ ] 6.9 Run `python3 scripts/check_delta_coverage.py` and confirm it passes. It must report the `geometry-layers` MODIFIED requirement as checked, with no dropped scenarios — the delta keeps both original scenarios and adds a third.
- [ ] 6.10 Run `openspec validate access-openings-must-pass-the-model` and confirm it passes.
