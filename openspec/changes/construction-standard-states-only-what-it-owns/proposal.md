...
Seven ruleset documents and `TODO.md`. **Thirteen rules are retired, three are trimmed, four sentences move to the documents that own them, and five citations are retargeted.** `tasks.md` carries the edit-by-edit coverage.
...
- **`CMP-009` and `CMP-010`.** They restate the `1 AP` cost a third time. Correcting that is `05-construction-components.md`'s change to make, not this one — see Out of Scope.
...
- **`system/proposal-review.md:182`,** where a worked example reads `≤ what fits the Deployment Volume (SCS-003, DEP-003)`. It needs `VEH-001` in place of `SCS-003`, on its own non-proposal branch (`system/workflow.md`, Git Workflow). **This is why `TODO.md` moves with the ruleset and `system/` does not:** `scripts/check_todo_quotes.py` couples `TODO.md` to `docs/` mechanically, so leaving it behind fails `preflight` on this very change; `system/` prose is not checked against `docs/` by anything and its repair is not part of applying this one.
...
- **`VEH-015` and `DEP-005` citing `CORE-001` for a sentence it no longer contains.** Both cite it for "what must fit is the Unit Base"; `core-states-only-what-it-owns` removed that paragraph from `CORE-001` naming `SCS-005` as its owner, and this change retires `SCS-005`. Neither rule becomes wrong — `CORE-001` still defines the Unit Base they measure against — but both citations now aim past their sentence. Found while checking what the `SCS-005` retirement leaves behind (`design.md`, Decision 3); repairing them belongs to `08-vehicles.md` and `06-deployment.md`.
- **The `1 AP` cost restated by `CMP-009` and `CMP-010`.** Three documents state it; this change removes one. `CORE-007` is the owner and `05-construction-components.md` should point at it, but widening this change into that document mixes two reviews.
