...
`system/proposal-review.md:182` cites `SCS-003` in a worked Range chain. Nothing checks `system/` against `docs/`, the line is prose rather than a quote, and repairing it is a `system/` edit that needs a branch and no proposal — so it was recorded Out of Scope while #104 was written.

**That was reversed, and section 19 carries the fix.** Two things settled it. No gate refuses the edit: `Branch name follows the convention` only requires that a branch touching `docs/` be named for its single change, `Docs require OpenSpec proposal` only that the proposal be complete, and the `PreToolUse` hook gates `CHANGELOG.md`, `docs/*.md` and `**Version:**` headers — `system/` is none of them, and `system/repository-strategy.md`'s Branch Naming table is a convention rather than a check. And "nothing checks it" is the argument *for* fixing it now rather than against: a stale citation no tool will ever flag is one that survives indefinitely, and the pull request retiring `SCS-003` is the last moment anyone is looking at both ends of it.

`TODO.md` still rides along for a different and stronger reason — `scripts/check_todo_quotes.py` couples it to `docs/` mechanically, so leaving it behind fails `preflight` on this very change.
