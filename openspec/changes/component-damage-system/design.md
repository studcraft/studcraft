...
**`CBT-008` and `CBT-013`'s hand-offs are updated to route through the new docs; `CBT-009`/`MAT-017`'s duplicate rule is consolidated.**
Reviewing the full set of documents this change touches surfaced two pre-existing organizational issues, unrelated to the new mechanism itself but directly affected by it: `CBT-008` (Defender Resolution) and `CBT-013` (Armour) both hand off to `13-materials.md` as if that document still owns resolution — after this change, it doesn't; `16-damage-system.md` does, and Materials only describes the physical result. Separately, `CBT-009` and `MAT-017` already assert the same "prefer physical representation" rule almost verbatim, independently of each other, predating this change. Alternative considered: leave both as-is, since neither is caused by this proposal. Rejected — this proposal is what makes the hand-off target stale and is a natural, low-risk moment to fix the pre-existing duplication too, rather than adding a third near-duplicate assertion in the new doc on top of the existing two.

**`component-damage` and `damage-resolution` ship as one document, not two.**
Alternative considered: one `docs/*.md` file per capability (`16-component-damage.md` + `17-damage-resolution.md`), mirroring the OpenSpec `specs/` folder structure exactly. Rejected — the two capabilities are unusually tightly coupled (Resistance is meaningless without the Geometry Check that reads it; the Geometry Check is meaningless without the Damage Roll it feeds), so splitting them would force a reader to jump between two files to follow one mechanic end to end. `weapon-construction-system` already established that an OpenSpec change's capability boundaries don't have to match its reader-facing document boundaries (it shipped `weapon-construction` and `weapon-capacity` as sections of one `10-weapons.md`). This change follows the same pattern: one `docs/16-damage-system.md`, two labeled sections, two `openspec/specs/` capabilities underneath.
