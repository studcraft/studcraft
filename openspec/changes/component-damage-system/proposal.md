...
- Affected documents: one new `docs/16-damage-system.md`, covering both capabilities as two clearly labeled sections rather than two files (see design.md — the two capabilities are too tightly coupled to read well split apart, and `weapon-construction-system` already established the precedent of one document spanning multiple capabilities); `docs/11-combat.md` (CBT-007 gains an exception, CBT-011 needs updating to allow Impact Strength, CBT-008/CBT-013's hand-offs are updated to route through the new document, CBT-009's duplicate "prefer physical representation" rule is consolidated); `docs/13-materials.md` (MAT-003, MAT-004 reworded to derive their hit count from Resistance instead of asserting it per material; MAT-011's "future armour rules" promise is fulfilled by the Geometry Check and Damage Roll; MAT-016 Cover is explicitly untouched; MAT-017's duplicate rule is consolidated); `docs/14-glossary.md` (new terms: Component, Resistance, Impact Strength cross-reference, Component State); `README.md` and `scripts/generate_site_docs.py` (add the new document to the repository index/site build — also fixes a pre-existing gap where `15-geometry-layers.md` was never added to `README.md` when it shipped).
