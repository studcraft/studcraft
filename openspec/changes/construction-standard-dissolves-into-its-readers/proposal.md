...
- **`SCS-013` — Platforms.** Both halves are stated: `MOVE-014` closes with "Physical construction determines accessibility", and `MOVE-015` places a falling unit "at the first surface that physically supports it". A permissive rule that grants what the plastic already grants is Principle 1 restated for one noun. Cited by nothing.
...
- **No gameplay value.** No distance, cost, dimension, threshold, capacity or state changes. Every requirement the five rules stated is still stated, in the rule that reads it, except the two whose text was already stated twice.
...
- **The Universal Rule's third level.** `02-core-rules.md` ranks Foundations, Core Rules, **Construction Standards**, Scenario Rules, and `03-game-flow.md` repeats the list. The level names a class of rule, not this file — `CORE-001` explicitly notes that "a system document … is not a level in it" — and `05-construction-components.md` and `10-weapons.md` hold construction standards after this change exactly as before it.
