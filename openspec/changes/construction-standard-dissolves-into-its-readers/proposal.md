...
- **The Universal Rule's third level, as a level.** `02-core-rules.md` still ranks Foundations, Core Rules, **Construction Standards**, Scenario Rules, and `03-game-flow.md` still repeats the list. **What does change is that the level now states its own membership**, in one sentence: a Construction Standard is any rule stating how a legal model or piece of terrain must be built, wherever it is written. That was not a cosmetic addition — the deleted document's `# Purpose` was the only text mapping the term onto files, so without it a conflict between a scenario rule and `CMP-018` was undecidable while `FLOW-013` went on enforcing the term (`design.md`, Decision 5). Stating membership by test also keeps the slope and stair requirements at level 3 after they move into `07-movement.md`.
