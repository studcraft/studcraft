...
- **`assets/IMAGES.md`.** Two rows go stale and neither can be repaired from this branch, which may touch `docs/*.md` and its own change directory only (`system/repository-strategy.md`, Branch Naming). The `CORE-002` row specifies an image of "the four directions (forward, rear, left, right) and firing arcs labelled" — content `CORE-002` no longer holds, and firing arcs no longer anywhere. The `CORE-001` row says the vertical projection is "taken across the 4-stud front", a phrase `CORE-001` stops using. `scripts/lint_ruleset.py` does not catch either: it checks that the rule ID exists, and both still do. **The `CORE-002` row should be re-aimed at `INF-001`, which now owns the front edge, and its firing-arc clause dropped.** A kebab-case follow-up branch, or fold it in if you would rather have one pull request.
