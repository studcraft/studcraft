...
- [x] 15.21 `git status --short` — now **nine** modified files: `docs/01-foundations.md`, `docs/02-core-rules.md`, `docs/04-construction-standard.md`, `docs/05-construction-components.md`, `docs/08-vehicles.md`, `docs/10-weapons.md`, `docs/12-melee.md`, `docs/14-glossary.md` and `docs/16-damage-system.md`. **`README.md` and `assets/IMAGES.md` must NOT appear** — they are a separate change (see this section's preamble). If either shows as modified, stop and report it.

  Verified after the fact, against `main` post-merge rather than a live proposal-branch diff (this box was left unticked when the proposal branch merged, which blocked archiving): `git status --short` on `main` is clean, all nine files carry the change's text (`grep -rn "projection" docs/` returns no hits, `CORE-004`'s only remaining occurrence is its own heading, `occupies two or more Unit Bases` is 0 in `docs/02-core-rules.md`), `README.md` and `assets/IMAGES.md` are untouched, `scripts/lint_ruleset.py` and `scripts/check_task_anchors.py core-states-only-what-it-owns` both pass.
