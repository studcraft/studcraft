...
Nine ruleset documents, `TODO.md` and one `system/` document. **Thirteen rules are retired, seven are trimmed, four sentences move to the documents that own them, and eight citations are retargeted or added.** `tasks.md` carries the edit-by-edit coverage; sections 1–16 are the change as proposed, section 17 the repairs its own audit found, and section 18 the seven changes the review of pull request #104 asked for.
...
- **`CORE-002` — Facing.** Loses the `Shield direction` item from its `Facing determines:` list, and the glossary's *Facing* entry with it. Asked for by the second review of #104, and it closes the loop the previous change left open: that change kept the bullet because nothing else in `docs/` stated shield direction, and `CMP-014` now does. Facing does not decide which way a shield protects — where the shield physically stands does.
- **`CMP-012` — Cargo Bays.** Gains the only citation this change adds. "Capacity is determined entirely by the available internal space" is what the retired `SCS-014` said, and `CMP-012` named no owner for it; it now points at `TRN-003` and `TRN-019`. Found by the semantic sweep the second review asked for (`design.md`, Decision 14) — the one gap in it, and a missing citation rather than a missing rule.
...
- **Any further `CORE-002` rework.** The `Shield direction` bullet is removed in section 20 and the glossary's *Facing* entry now routes that question to `CMP-014`; nothing else about `CORE-002` is touched. No abstract "shield facing" mechanic is created — the plastic decides (`design.md`, Decision 12).
